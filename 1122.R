...
# 데이터의 구조(행과 열의 개수, 각 열별 데이터의 요약내용용)
str(iris)
...
# df의 w열이 빈 문자열이 아니고, z열의 유한한 값만 갖는 행만 추출
df2 = filter(df, nzchar(W) & is.finite(Z))

# df의 w열이 빈 문자열이 아니고, z열의 유한한 값인 행과 특정 열(w, x열) 추출
...
df3$W[df3$W == ""] = "Unknown"  # 빈 문자열을 "Unknown"으로 대체
