...
ggplot(data = df1, aes(x=부서,y=매출))+
  geom_bar(stat="identity", fill= c("tomato","gold","green"), color="black")+
  labs(title = "팀별 영업실적", x="부서명",y="매출(억원)")
...
#3분기 실적 추가
#조건1. 3분기 실적추가, 조건2.gdRdriver 패키지의 팔레트에 있는 색상을 막대에 설정,
#조건3. 그래프 제목: 부서별분기별 영업실적, x축제목: 부서명, y축 제목: 영업매출(억원)
dept = c("영업1팀","영업2팀","영업3팀")
sales = c(123,223,1)
...
ggsave("D:/R_data/graph1.png",width = 10, height = 13, units="cm")
